...
def load_data(sales_path: str, demographics_path: str, sales_column_selection: List[str]):
...
def make_plot(y_actual, y_predicted, out_path: pathlib.Path, n_outliers: int = 5):
    ...
    if len(errors) > 0:
        ...
        ax.scatter(y_actual.loc[top_idx], y_predicted.loc[top_idx], color='red', s=60, label='Outliers')
        for i in top_idx:
            ax.annotate(f"{int(errors.loc[i]):,}", (y_actual.loc[i], y_predicted.loc[i]),
...
def main():
    ...
    if not model_file.exists():
        raise SystemExit(f"Model file not found at {model_file} - run training first or point MODEL_PATH to the model artifact")
    ...
    # Left: table
    axes[0].axis('off')
    table = axes[0].table(cellText=metrics_table.values,
    ...
    if n_outliers > 0:
        top_idx = errors.nlargest(n_outliers).index
        ax.scatter(y_test.loc[top_idx], pd.Series(y_pred_test, index=y_test.index).loc[top_idx],
                   color='red', s=60, label='Outliers')
        for i in top_idx:
            ax.annotate(f"{int(errors.loc[i]):,}", (y_test.loc[i], pd.Series(y_pred_test, index=y_test.index).loc[i]),
                        textcoords="offset points", xytext=(5,5), ha='left', fontsize=8)
