  ...
  const handleCsvDataProcessed = async (data: CsvData[]) => {
    ...
    try {
      const predictions = await Promise.all(
        data.map(async (property, index) => {
          try {
            ...
            console.error(`Error predicting property ${index + 1}:`, err)
      ...
      if (successfulPredictions.length !== predictions.length) {
        setError(`Failed to predict ${predictions.length - successfulPredictions.length} properties`)
      }
    } catch (err) {
      setError('Failed to process CSV predictions. Please check if the backend server is running.')
    ...
    <Stack className={`w-full gap-12 relative landing landing-${theme ? 'dark' : 'light'}`}>
      ...
      <Modal isOpen={resultsModalOpen} onClose={() => setResultsModalOpen(false)} size="xl">
      ...
      <Stack className="absolute left-0 right-0 bottom-0 top-0 pt-20 items-center overflow-y-auto">
        ...
        <Box className="landing-2 w-full max-w-2xl flex justify-end mt-12 px-6">
          <Button rounded="rounded-full" className="px-6 py-2.5" onClick={() => setCsvModalOpen(true)}>
            <Translations text="Upload CSV" />
          </Button>
        </Box>
